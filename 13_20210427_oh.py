...
game_bar_1 = pygame.image.load("C:/PythonWorkspace/flema/게임3_1.jpg") # 바 그림 불러오기
game_bar_1_size = game_bar_1.get_rect().size # 바 이미지의 크기를 구해옴
game_bar_1_w = game_bar_1_size[0] # 바 가로 크기
game_bar_1_h = game_bar_1_size[1] # 바 세로 

game_bar_2 = pygame.image.load("C:/PythonWorkspace/flema/게임3_2.jpg") # 바 그림 불러오기
game_bar_2_size = game_bar_2.get_rect().size # 바 이미지의 크기를 구해옴
game_bar_2_w = game_bar_2_size[0] # 바 가로 크기
game_bar_2_h = game_bar_2_size[1] # 바 세로 

game_bar_3 = pygame.image.load("C:/PythonWorkspace/flema/게임3_3.jpg") # 바 그림 불러오기
game_bar_3_size = game_bar_3.get_rect().size # 바 이미지의 크기를 구해옴
game_bar_3_w = game_bar_3_size[0] # 바 가로 크기
game_bar_3_h = game_bar_3_size[1] # 바 세로 

game_bar_4 = pygame.image.load("C:/PythonWorkspace/flema/게임3_4.jpg") # 바 그림 불러오기
game_bar_4_size = game_bar_1.get_rect().size # 바 이미지의 크기를 구해옴
game_bar_4_w = game_bar_4_size[0] # 바 가로 크기
game_bar_4_h = game_bar_4_size[1] # 바 세로 

game_bar_5 = pygame.image.load("C:/PythonWorkspace/flema/게임3_5.jpg") # 바 그림 불러오기
game_bar_5_size = game_bar_1.get_rect().size # 바 이미지의 크기를 구해옴
game_bar_5_w = game_bar_5_size[0] # 바 가로 크기
game_bar_5_h = game_bar_5_size[1] # 바 세로 
...
game_down_x =[305, 445, 585, 725, 865] # 떨어지는 바 x 값 집합
...
while running: # True 시 진행 False 시 종료
    ...
    dt = clock.tick(30) #게임 진행하는동안 60프레임으로 고정 *이거 나중에 필요함
    ...
    if album_num == 1:
        ...
        game_down_speed = 17 # 떨어지는 바 속도
    elif album_num == 3:
        game_down_speed = 25 # 떨어지는 바 속도
      ...
      #           hee_x_pos =   
        if event.type == pygame.KEYDOWN: # 키보드 입력 이벤트 발생하려면 필요
                ...
                if event.key == pygame.K_LEFT:
                    if game >=1 :
                        hee_x_pos = game_bar_1_w
                        hee_y_pos = game_bar_1_h
                        hees.append([hee_x_pos,hee_y_pos])
                        mo = 1
        if event.type == pygame.KEYDOWN: # 키보드 입력 이벤트 발생하려면 필요
            
                if event.key == pygame.K_DOWN:
                    if game >=1 :
                        hee_x_pos = game_bar_2_w
                        hee_y_pos = game_bar_2_h
                        hees.append([hee_x_pos,hee_y_pos])
                        mo = 2
        if event.type == pygame.KEYDOWN: # 키보드 입력 이벤트 발생하려면 필요
                ...
                if event.key == pygame.K_RIGHT:
                    if game >=1 :
                        hee_x_pos = game_bar_3_w
                        hee_y_pos = game_bar_3_h
                        hees.append([hee_x_pos,hee_y_pos])
                        mo = 3
        if event.type == pygame.KEYDOWN: # 키보드 입력 이벤트 발생하려면 필요
                ...
                if event.key == pygame.K_UP:
                    if game >=1 :
                        hee_x_pos = game_bar_4_w
                        hee_y_pos = game_bar_4_h
                        hees.append([hee_x_pos,hee_y_pos])
                        mo = 4
        if event.type == pygame.KEYDOWN: # 키보드 입력 이벤트 발생하려면 필요
                ...
                if event.key == pygame.K_SPACE:
                    if game >=1 :
                        hee_x_pos = game_bar_5_w
                        hee_y_pos = game_bar_5_h
                        hees.append([hee_x_pos,hee_y_pos])
                        mo = 5                
...
#                        if game_down_rect.colliderect(game_bar_rect):
#                             print("충돌 했어요")
#                        else:
#                             print("미스에요 ㅋㅋ")
#            mo = 0
#        if event.type == pygame.KEYUP:
#                mo = 1
#        pass  빈칸으로 두면 오류 발생해서
    


    game_downs = [ [g[0], g[1] + game_down_speed] for g in game_downs] # x, y 값 갱신
    game_downs = [ [g[0], g[1]] for g in game_downs if g[1] < 620] # y 값이 바닥에 닿으면 사라짐
    ...
    screen.blit(album,(515,277.5)) #앨범 불러오기
    ...
    if game >=1: # 변수가 1이상 일 때
        ...
        screen.blit(game_bar_1, (305, 605)) # 게임 바 불러오기
        screen.blit(game_bar_2, (445, 605)) # 게임 바 불러오기
        screen.blit(game_bar_3, (585, 605)) # 게임 바 불러오기
        screen.blit(game_bar_4, (725, 605)) # 게임 바 불러오기
        screen.blit(game_bar_5, (865, 605)) # 게임 바 불러오기
        screen.blit(back_button, (screen_w - back_button_w, 0)) # 게임 뒤로가기 버튼
        
        if mo == 1:
            for hee_x_pos,hee_y_pos in hees:
                 screen.blit(hee, (305,605))
        if mo == 2:
            for hee_x_pos,hee_y_pos in hees:
                 screen.blit(hee, (445,605))
        if mo == 3:
            for hee_x_pos,hee_y_pos in hees:
                 screen.blit(hee, (585,605))
        if mo == 4:
            for hee_x_pos,hee_y_pos in hees:
                 screen.blit(hee, (725,605))
        if mo == 5:
            for hee_x_pos,hee_y_pos in hees:
                 screen.blit(hee, (865,605))



        timer = game_font.render(str(int(elapsed_time)), True, (255, 255, 255)) # 시간 표시 폰트 이미지 불러오기
        screen.blit(timer, (0, 0)) # 게임 중 시간 표시

        
        
        #예시1
        for koo in range(1,6):
    ...
    elapsed_time = (pygame.time.get_ticks() - start_ticks) / 1000 # 경과 시간 계산 단위는 1초
