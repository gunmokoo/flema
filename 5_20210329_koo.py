...
game_bar_h = game_bar_size[1] # 바 세로 크기

game_down = pygame.image.load("C:/PythonWorkspace/flema/게임4.jpg") # 떨어지는 바 그림 불러오기
game_bar_size = game_bar.get_rect().size # 떨어지는 바 이미지의 크기를 구해옴
game_bar_w = game_bar_size[0] # 떨어지는 바 가로 크기
game_bar_h = game_bar_size[1] # 떨어지는 바 세로 크기
        ...
        if event.type == pygame.MOUSEBUTTONUP and event.button == LEFT: # 마우스  왼쪽 클릭 이벤트 발생하면
            ...
            if 435  <= mousepos[0] <= 435 + album_w and 247.5 <= mousepos[1] <= 247.5 + album_w: # 엘범을 클릭 했을때
                ...
                if  435 + (album_h / 2) + ( no_button_h /2) <= mousepos[0] <= 435 + (album_h / 2) + ( no_button_h /2) + no_button_w \
                    and album_w <=mousepos[1] <= album_w + no_button_h: # 아니오 버튼 클릭 좌표
                    yes_no = 0 # 변수에 0을 저장
                if 435 + (album_h / 2) - ( yes_button_h *2.5) <= mousepos[0] <= 435 + (album_h / 2) - ( yes_button_h *2.5) + yes_button_w \
                    and album_w <=mousepos[1] <= album_w + yes_button_h: # 예 버튼 클릭 좌표
                    yes_no = 0 # 변수에 0을 저장
                    game += 1
