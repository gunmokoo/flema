...
while running: # True 시 진행 False 시 종료
    ...
    for event in pygame.event.get(): # 이거해야 이벤트 할수있어(이거 지우는 얘는 없겠지)
        ...
        if event.type == pygame.MOUSEBUTTONUP and event.button == LEFT: # 마우스  왼쪽 클릭 이벤트 발생하면
            ...
            if 435  <= mousepos[0] <= 435 + album_w and 247.5 <= mousepos[1] <= 247.5 + album_w:
                
                

    screen.blit(yes_button, ( 435 + (album_h / 2) - ( yes_button_h *2.5)    , album_w ))
    screen.blit(no_button, ( 435 + (album_h / 2) + ( yes_button_h /2)    , album_w ))
