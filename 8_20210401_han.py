...
game_down_y_pos = 0 # 떨어지는 바 y 값
game_down_speed = 10 # 떨어지는 바 속도
game_down_x_pos = 0 # 떨어지는 바 x 값
game_downs =[305, 445, 585, 725, 865] # 떨어지는 바 x 값 집합

game_down_xx_pos = sample(game_downs , 2)  # 시작 x 값 뽑기
game_down_x_pos = game_down_xx_pos[0] # 시작 x 값 정의 
...
while running: # True 시 진행 False 시 종료
    ...
    if game >=1: # 변수가 1이상 일 때
        ...
        screen.blit(back_button,(0,0)) # 게임 뒤로가기 버튼
        if 0 <= mousepos[0] <= back_button_h and 0 <=mousepos[1] <= back_button_w: # 뒤로가기 버튼 클릭했을때
            game = 0 # 게임 변수를 0으로 정의
            game_down_y_pos = 0 # y좌표 초기화
            game_down_xx_pos = sample(game_downs , 2)  # 시작 x 값 뽑기
            game_down_x_pos = game_down_xx_pos[0] # 시작 x 값 재정의 
        screen.blit(game_down,(game_down_x_pos,game_down_y_pos)) # 떨어지는 바 불러오기


        game_down_y_pos += game_down_speed # 떨어지는 바 떨어뜨리기
        if game_down_y_pos > screen_h:
            shuffle(game_downs) # game_downs 리스트 섞기
            game_down_xx_pos = sample(game_downs , 2) # 섞은 x좌표에서 2개 뽑아오기
            game_down_x_pos = game_down_xx_pos[0] # x값의 첫번째값 가져오기
            game_down_y_pos = 0 # y좌표 정의
